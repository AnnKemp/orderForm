...
if ($_SERVER["REQUEST_METHOD"] == "POST"){  // --------start post data ---------------------------------------------------------------------------------------------
    ...
    function test_input(string $data) // trimmen, stripslashes and specialchars  // is this old fashioned?
    {
        $data = trim($data);
    ...
    if (empty($_POST["zipcode"])) {
        ...
        if(!empty($_POST['products'])) {
            ...
            foreach ($contents as $x => $x_value) {
                ...
                // set the expiration date to one hour ago to delete the cookie
            ...
            // find a way to set a cookie
            setcookie("$cookie_email", "$price", time() + (86400 * 30)); // 86400 = 1 day * 30 is for a month
            //setcookie("cookie_email", "$price", time() + (86400 * 30)); // 86400 = 1 day * 30 is for a month
//$_COOKIE["email"]=$totalValue;  // om totaal bestelling in cookies op te slaan ook email en die checken om user te identificieren
...
//------------------------------------------------------ calculate the delivery time after choosing the mode: normal or express ---------------------------------------------
         ...
         if(!empty($_POST['delivery'])) { // check of they have chosen a delivery way! if true:
           ...
           if (isset($delivery) && $delivery=="normal"){ // if isset and they have choosen normal delivery
               ...
               $time=$time+2;
                echo '<p class="textAbove">Your order will be delivered at: ' . date("$time:ia")."</p>";
                $delivery_time=date("$time:ia");
           }
            elseif (isset($delivery) && $delivery=="express"){ // if isset and they have choosen express delivery
                ...
                echo '<p class="textAbove">Your order will be delivered at: ' . date("$hours:$minutes")."</p>";
